...
**Document version:** 2.9 (2026-09-14) — memory_conflict storage + BDS-DF-RF-001 verification wiring
...
| BDS-DF-RF-001 Receipt-to-Finding spine | `/api/v1/df-rf` | `POST /api/v1/df-rf`, `GET /api/v1/df-rf/review-feed` | Storage for `df_rf_evidence_link.v1`/`df_rf_finding_candidate.v1`/`df_rf_disposition.v1` (RFC-DF-RF-01, `forge_contract_core`). Generalizes the `llm-intel/pending-records` ingest *pattern* (idempotent, `stable_hash()`-based) per OD-03, not its tables. `candidate_id` is a deterministic `uuid5` from a producer-supplied `natural_key`, computed here, never trusted from the caller. `verification_status` on evidence links is likewise computed here against DataForge's own storage (`forge_check_run_receipts_v1`, `memory_conflicts`) rather than accepted from the reporting system. Terminal candidates only (OD-02) — no promotion endpoint exists. |
| Forge Memory spine (FMEM-02) | `/api/v1/memory` | `POST /api/v1/memory/{episodes,claims,facts,retrieval-receipts,use-receipts,conflicts}`, `POST /api/v1/memory/facts/{memory_id}/supersede`, `GET /api/v1/memory/facts`, `DELETE /api/v1/memory/{family}/{artifact_id}` | Authenticated, tenant-scoped, bitemporal memory storage. `/conflicts` (added 2026-09-14 for `BDS-FMEM-OPCOURT-001`) stores `memory_conflict.v1` — never resolves it; a conflict with `operator_review_required: true` is meant to reach a human via `BDS-DF-RF-001`'s disposition spine (`df_rf_ingest.py`'s `verification_status` check queries this table directly). |
