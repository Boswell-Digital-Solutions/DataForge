...
from app.api.healing_proposal_router import (
    router as healing_proposal_router,
)  # Self-healing code-fix proposals (operator accept/reject)
...
app.include_router(healing_proposal_router)  # Self-healing code-fix proposals (store, list, get, decide)
...
async def readiness_check():
    ...
    try:
        ...
        if redis:
            ...
            # Surface scheme + host (password masked) so readiness is self-
            # diagnosing: reveals rediss:// vs redis:// and localhost vs cloud.
            from app.config import REDIS_URL as _ru
            _endpoint = "(unset)"
            if _ru:
                try:
                    _sch, _rest = _ru.split("://", 1)
                    if "@" in _rest:
                        _cr, _hp = _rest.rsplit("@", 1)
                        _endpoint = f"{_sch}://{_cr.split(':', 1)[0]}:***@{_hp}"
                    else:
